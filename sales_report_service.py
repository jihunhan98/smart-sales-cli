...
from validators import validate_not_empty, validate_min_length
...
def register_report(customer_id: str, activity_date: str, content: str) -> dict:
    ...
    if not validate_not_empty(activity_date):
        return {"success": False, "message": "활동 날짜는 비울 수 없습니다."}
    if not validate_min_length(content, 5):
        return {"success": False, "message": "내용은 최소 5자 이상 입력해야 합니다."}
